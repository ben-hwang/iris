...
def cropImage(filename):
	...
	h1 = center[1]+offset
	# print(w0, w1, h0, h1)
	crop_img = img[int(h0):int(h1), int(w0):int(w1)]
	# cv2.imshow('cropped', crop_img)
	# cv2.waitKey(0)
	cv2.imwrite(filename[:-4]+'_cropped'+'.jpg', crop_img)

def colorML(filename):
	#read image
	y = h = w = x = 500
	img = cv2.imread(filename)
...
# ===========================================================================================
	...
	# Load data from path 
	df = pd.read_csv('color_dataset.csv', low_memory=False)
...
takePics()
cropImage('picture_0.jpg')
colorML('picture_0_cropped.jpg')
